...
module.exports = (gen, classInfo, version, options) => {
  ...
  if (generic && generic.length === 2) {
    ...
    gen('    const encodeKey = compile(\'%s\', %j, classMap, version, %j); encodeKey(key, encoder, classMap);', keyId, genericKeyDefine, options);
    gen('    const encodeValue = compile(\'%s\', %j, classMap, version, %j); encodeValue(value, encoder, classMap);', valueId, genericValueDefine, options);
    gen('  }\n  } else {');
    gen('  for (const key in obj) {');
    gen('    const value = obj[key];');
    const convertor = utils.converts[genericKeyDefine.type];
    if (convertor) {
      gen('    const encodeKey = compile(\'%s\', %j, classMap, version, %j); encodeKey(%s(key), encoder, classMap);', keyId, genericKeyDefine, options, convertor);
    } else {
      gen('    const encodeKey = compile(\'%s\', %j, classMap, version, %j); encodeKey(key, encoder, classMap);', keyId, genericKeyDefine, options);
    }
    gen('    const encodeValue = compile(\'%s\', %j, classMap, version, %j); encodeValue(value, encoder, classMap);', valueId, genericValueDefine, options);
    ...
    gen('      compile(fnKey, value, classMap, version, %j)(value.$, encoder, classMap);', options);
    gen('    } else {');
