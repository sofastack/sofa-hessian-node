...
function compile(uniqueId, info, classMap, version, options) {
  ...
  const gen = codegen([ 'obj', 'encoder', 'classMap' ], 'encode');
  // 默认值
  ...
  if (info.isArray) {
    ...
    if (bufferType[type] && arrayDepth === 1) {
      ...
      gen('  compile(\'%s\', %j, classMap, version, %j)(item, encoder, classMap);', uniqueId, item, options);
      gen('}');
    ...
    gen('  compile(fnKey, obj, classMap, version, %j)(obj.$, encoder, classMap);', options);
    gen('  return;');
    gen('}');
    gen('if (encoder._checkRef(obj)) { return; }');
    ...
    if (version === '1.0') {
      ...
      for (const key of keys) {
        ...
        gen('compile(\'%s\', %j, classMap, version, %j)(obj[\'%s\'], encoder, classMap);', uniqueId, attr, options, key);
      }
      gen('encoder.byteBuffer.put(0x7a);');
    } else {
      gen('const ref = encoder._writeObjectBegin(\'%s\');', type);
